...
static void mbFini(rpmMacroBuf mb, rpmMacroEntry me, MacroExpansionData *med)
{
	...
	mb->expand_trace = 1;
    if (mb->expand_trace) {
	const char *mbuf = mb->buf.c_str();
	printExpansion(mb, me, mbuf + med->tpos, mbuf + mb->buf.size());
    }
	...
	macroFunc f, void *priv, int nargs, int level, int flags)
{
	...
	auto pe = mc->tab.insert({string(n), nullptr});
	mep = &pe.first->second;
