...
static int ndb_Open(rpmdb rdb, rpmDbiTagVal rpmtag, dbiIndex * dbip, int flags)
{
	...
	*dbip = NULL;
	...
	rpmpkgdb pkgdb = (rpmpkgdb)rdb->db_pkgs->dbi_db;
	unsigned int id;
	rpmidxdb idxdb = 0;
	if (!ndbenv->pkgdb) {
	    ndb_Close(dbi, 0);
	...
	if (!ndbenv->xdb) {
	    ...
	    ioflags = O_RDWR;
	    rc = rpmxdbOpen(&ndbenv->xdb, pkgdb, path, ioflags, rdb->db_perms);
	...
	if (rpmidxOpenXdb(&idxdb, pkgdb, ndbenv->xdb, rpmtag, oflags)) {
...
static rpmRC ndb_pkgdbPut(dbiIndex dbi, dbiCursor dbc,  unsigned int *hdrNum, unsigned char *hdrBlob, unsigned int hdrLen)
{
    ...
    if (hnum == 0) {
	rc = rpmpkgNextPkgIdx(pkgdb, &hnum);
	if (!rc && ndbenv->hdrNum == hnum)
	    setdata(dbc, hnum, 0, 0);
    }

    if (!rc)
	rc = rpmpkgPut(pkgdb, hnum, hdrBlob, hdrLen);
...
static rpmRC ndb_pkgdbIter(dbiIndex dbi, dbiCursor dbc, unsigned char **hdrBlob, unsigned int *hdrLen)
{
    ...
    if (!dbc->list) {
	rc = rpmpkgList(pkgdb, &dbc->list, &dbc->nlist);
	if (rc)
	    return rc;
	dbc->ilist = 0;
    }
	...
	*hdrBlob = 0;
	...
	rc = rpmpkgGet(pkgdb, hdrNum, hdrBlob, hdrLen);
...
static rpmRC ndb_idxdbIter(dbiIndex dbi, dbiCursor dbc, dbiIndexSet *set)
{
	...
	rc = rpmidxList(idxdb, &dbc->list, &dbc->nlist, &dbc->listdata);
	if (rc)
	    return rc;
	dbc->ilist = 0;
    }
	...
	pkglist = 0;
	pkglistn = 0;
	rc = rpmidxGet(idxdb, k, kl, &pkglist, &pkglistn);
	if (rc && rc != RPMRC_NOTFOUND)
	    break;
	dbc->ilist += 2;
	if (!rc && pkglistn) {
...
static rpmRC ndb_idxdbGet(dbiIndex dbi, dbiCursor dbc, const char *keyp, size_t keylen, dbiIndexSet *set, int searchType)
{
    ...
    rc = rpmidxGet(idxdb, (const unsigned char *)keyp, keylen, &pkglist, &pkglistn);
    if (!rc)
	addtoset(set, pkglist, pkglistn);
    return rc;
}
